...
import data
import metric
import models as model_definition #### none define path
import os
import storage
import torch
...
def test(model: torch.nn.Module, data_generator: torch.data.Dataloader, entities_count: int,
         summary_writer: tensorboard.SummaryWriter, deviec: torch.device, epoch_id: int, metric_suffix: str) -> METRICS:
    examples_count = 0.0
    hits_at_1 = 0.0
    hits_at_3 = 0.0
    hits_at_10 = 0.0
    ...
    for head, relation, tail in data_generator:
        ...
        triplets = torch.stack((all_entities, relation, tails), dim=2).reshape(-1,3)
